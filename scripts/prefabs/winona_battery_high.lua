...
local function StartBattery(inst)
	...
	inst.Light:Enable(true) --Mod control
    --end 由于打开后未连接设备 不会发光导致无法知道已打开 故删除
...
local function StopBattery(inst)
    inst.Light:Enable(false) --Mod control
...
local function OnBurnt(inst)
    ...
    inst:RemoveComponent("machine") --Mod control
...
local function OnLoad(inst, data, ents)
    if data ~= nil then
        ...
        elseif not inst.components.fueled:IsEmpty() then
            ...
            inst.AnimState:PlayAnimation("idle_charge", true)
            if inst.components.machine:IsOn() and not inst:IsAsleep() then -- Mod control
                StartIdleChargeSounds(inst)
            end
...
local function fn()
    ...
    inst.entity:AddLight() --Mod control
    ...
    inst.Light:SetColour(163 / 255, 255 / 255, 186 / 255);
	...
	inst._isupdating = inst:DoPeriodicTask(CHECK_FERT_TIME, CheckForFuel, 5)						 
    inst.OnSave = OnSave
    inst.OnLoad = OnLoad
